...
plt.xlabel('Date')
plt.ylabel('Count')
plt.title('LSTM Model Predictions for Ambulance Data (PyTorch)')
plt.legend()
plt.show()

# Forecast future values (next 30 days)
last_sequence = torch.tensor(scaled_data[-time_step:], dtype=torch.float32).unsqueeze(0) #.unsqueeze(-1)  # Last sequence for prediction
future_predictions = []
model.eval()
for _ in range(30):
...
plt.xlabel('Date')
plt.ylabel('Count')
plt.title('LSTM Forecast for Ambulance Calls (Next 30 Days) - PyTorch')
plt.legend()
plt.show()