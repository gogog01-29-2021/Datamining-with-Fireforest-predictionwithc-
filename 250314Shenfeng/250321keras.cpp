...
struct LSTMNet : torch::nn::Module {
    ...
    torch::Tensor forward(torch::Tensor x) {
        ...
        auto out = torch::relu(fc1->forward(out2.output.index({torch::indexing::Slice(), -1, torch::indexing::Slice()})));
        out = fc2->forward(out);
        return out;
    }
};
...
int main() {
    ...
    int epochs = 1; // For demo, use more for real training
