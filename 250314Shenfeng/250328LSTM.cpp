...
struct LSTMNet : torch::nn::Module {
    torch::nn::LSTM lstm;
    torch::nn::Linear fc;
    LSTMNet(int input_size, int hidden_size, int num_layers)
...
int main() {
    ...
    LSTMNet model(input_size, hidden_size, num_layers);
    model.train();
    ...
    std::ofstream out3("250328LSTM_future_results.csv");
    out3 << "Future_Predicted\n";
    for (size_t i = 0; i < future_actual.size(); ++i)
        out3 << future_actual[i] << "\n";
    out3.close();
    std::cout << "Results written to 250328LSTM_train_results.csv, 250328LSTM_test_results.csv, and 250328LSTM_future_results.csv" << std::endl;
    return 0;
}
